/******************************** GPR_STAK.C ********************************\
 *                               version 1.08.09.01
 * GPR_STAK reduces the size of digital ground penetrating radar data by
 * stacking traces.  A beginning trace and end trace must be specified
 * and the number of traces to stack.  The following computer storage
 ...
 *
 * Revision History:
 *  February 15, 1996  - Completed as a modification of GPR_CNDS.C
 *  February 16, 1996  - Added exit() if GetGprFileType() returns error.
 ...
 *       icc /F /xnovm /zfloatsync gpr_stak.c gpr_io.lib jl_util1.lib
 *     To compile for 80486 executable:
 *       icc /F /xnovm /zfloatsync /zmod486 gpr_stak.c gpr_io.lib jl_util1.lib
 *
 ...
 *     /DNDEBUG
 *
 * To run: GPR_STAK in_filename out_filename start end stack [/d /b]
 *
\****************************************************************************/
...
/*************************** Manifest constants ****************************/
#define GPR_STAK_VER "1.08.09.01"
...
void main (int argc,char *argv[])
{
    ...
    {   puts("\n");
        ...
        puts("");
        puts("###############################################################################");
        puts("   Usage: GPR_STAK in_filename out_filename start end stack [/d /b]");
        puts("     Computer code written by Jeff Lucius  USGS  lucius@usgs.gov");
		puts("   This program reduces the size of GPR files by stacking traces.");
        ...
        puts("     stack        - The number of traces to stack to construct a new trace.");
		...
		puts(" Examples: gpr_stak file1.dzt newfile1.dzt -1 -1 2");
        puts("           gpr_stak file1.dt1 newfile1.dt1 53 870 10 /b");
        printf("  Version %s - Last Revision Date: %s at %s\n",GPR_STAK_VER,DATETIME,__TIME__);
        puts("###############################################################################");
    ...
    {   printf("\nGPR_STAK - ERROR: input and output filenames are the same.\n");
        exit(2);
    }
	...
	/* Get the data, stack, and save to disk */
    printf("Stacking file %s to %s ...",in_filename,out_filename);
    switch (file_type)
	{   case DZT:  case MOD_DZT:
		{   char text[400],str[160];
            int  max_proc_hist_size = 400;
            unsigned char proc_hist[400];
            char name[12],antname[14], drive[_MAX_DRIVE], dir[_MAX_DIR];
            char fname[_MAX_FNAME], fext[_MAX_EXT];
            int  i,itemp;
            int  num_hdrs,num_samps,header_size,channel,selection,dot,icount;
            ...
            if ((end - start) < 1)
            {   printf("GPR_STAK - ERROR: (end - start) is less than 1.\n");
                exit(4);
            }
            if (stack < 1)  stack = 1;
            if (stack >= (end-start)) stack = end - start - 1;

            /* Allocate space for a block of traces (include stacked ones) */
            num_samps = hdr.rh_nsamp;
            disk_trace_bytes = (hdr.rh_bits/8) * num_samps;
            disk_chunk_bytes = disk_trace_bytes * hdr.rh_nchan;
            block_size = disk_chunk_bytes * stack;
            if ((v_ptr = (void *)malloc(block_size)) == NULL)
			{   puts("GPR_STAK - ERROR: unable to allocate storage for one chunk of DZT traces.");
                exit(4);
            }

            /* Allocate space for summing trace */
            d_ptr = (double *)malloc(num_samps * sizeof(double));
            if (d_ptr == NULL)
            {   puts("GPR_STAK - ERROR: unable to allocate storage for summing trace.");
                exit(4);
            }

            /* Open input file */
            if ((infile = fopen(in_filename,"rb")) == NULL)
            {   printf("GPR_STAK - ERROR: Unable to open input file %s\n",in_filename);
            ...
            {   printf("GPR_STAK - ERROR: Unable to open output file %s\n",out_filename);
            ...
            sprintf(str,"   file = %s  start = %d  end = %d  stack = %d\n",
                in_filename,start,end,stack);
            ...
            /* Note: Do not send pointers from hdr through this function!
                     SetDzt5xHeader() clears structure arrays before writing
                     to them! Other members are OK as they are sent by value.
             */
            ...
            /* Condense file.  File pointer must be at start of first chunk
               to get.  Read in desired trace chunk plus stacked ones. */
            dot = (end-start)/stack/10;
            if (dot < 1) dot = 1;
            icount = 0;
            trace = start;
            for (trace=start; trace<=end; trace+=stack,icount++)
            {   if (!(icount % dot)) printf(".");
                if (end - trace >= stack)    /* far enough from end? */
                {   fread(v_ptr,block_size,1,infile);
                } else                       /* get end trace */
                {   offset = (num_hdrs * header_size) +
                             (end * disk_chunk_bytes) + trace_offset ;
                    fseek(infile,offset,SEEK_SET);
                    fread(v_ptr,disk_trace_bytes,1,infile);
                    stack = 1;
                }

                /* check for markers and stack */
                switch (rh_dtype)
				{   case 0x0000:
                    {   unsigned char *uc_ptr;
                        int samp;

                        /* initialize stacking trace */
                        for (i=0; i<num_samps; i++) d_ptr[i] = 0.0;

                        /* set 1st sample of stacking trace */
                        d_ptr[0] = 0xFFFF;

                        /* set rest of samples of stacking trace */
                        for (i=0; i<stack; i++)
                        {   /* point to start of each chunk of traces */
                            uc_ptr = (unsigned char *)( (char *)v_ptr + (i * disk_chunk_bytes));

                            /* check for marker */
                            if (uc_ptr[1] == 0xE8)  d_ptr[1] = 0xE8;

                            /* point to start of requested channel for each chunk of traces */
                            uc_ptr = (unsigned char *)( (char *)v_ptr +
                                                         ( (i * disk_chunk_bytes) +
                                                           trace_offset ) );

                            /* add remaining samps into stacking trace */
                            for (samp=2; samp<num_samps; samp++)
                                d_ptr[samp] += (double)uc_ptr[samp] - 0x80U;
                        }

                        /* set 2st sample of stacking trace */
                        if (d_ptr[1] != 0xE8U)  d_ptr[1] = 0xF1U;

                        /* calculate average */
                        for (samp=2; samp<num_samps; samp++)
                        {    d_ptr[samp] /= stack;
                            d_ptr[samp] += 0x80U;
                            d_ptr[samp]  = _LIMIT(0.0,d_ptr[samp],0xFFU);
                        }

                        /* point to start of trace to save */
                        uc_ptr = (unsigned char *)((char *)v_ptr + trace_offset);

                        /* copy stacked trace into v_ptr */
                        for (samp=0; samp<num_samps; samp++)
                            uc_ptr[samp] = d_ptr[samp];
                    ...
                    {   unsigned short *us_ptr;
                        ...
                        for (i=0; i<stack; i++)
                        {   /* point to start of each chunk of traces */
                            us_ptr = (unsigned short *)( (char *)v_ptr + (i * disk_chunk_bytes));

                            /* check for marker */
                            if (us_ptr[1] == 0xE800U)  d_ptr[1] = 0xE800U;

                            /* point to start of requested channel for each chunk of traces */
                            us_ptr = (unsigned short *)( (char *)v_ptr +
                                                         ( (i * disk_chunk_bytes) +
                                                           trace_offset ) );

                            /* add remaining samps into stacking trace */
                            for (samp=2; samp<num_samps; samp++)
                                d_ptr[samp] += (double)us_ptr[samp] - 0x8000U;
                        }

                        /* set 2st sample of stacking trace */
                        if (d_ptr[1] != 0xE800U)  d_ptr[1] = 0xF100U;

                        /* calculate average */
                        for (samp=2; samp<num_samps; samp++)
                        {    d_ptr[samp] /= stack;
                            d_ptr[samp] += 0x8000U;
                            d_ptr[samp]  = _LIMIT(0.0,d_ptr[samp],0xFFFFU);
                        }

                        /* point to start of trace to save */
                        us_ptr = (unsigned short *)((char *)v_ptr + trace_offset);

                        /* copy stacked trace into v_ptr */
                        for (samp=0; samp<num_samps; samp++)
							us_ptr[samp] = d_ptr[samp];
                    ...
                    {   unsigned long *ul_ptr;
                        ...
                        /* set 1st sample of stacking trace */
                        d_ptr[0] = 0xFFFFFFFFUL;

                        /* set rest of samples of stacking trace */
                        for (i=0; i<stack; i++)
                        {   /* point to start of each chunk of traces */
                            ul_ptr = (unsigned long *)( (char *)v_ptr + (i * disk_chunk_bytes));

                            /* check for marker */
                            if (ul_ptr[1] == 0xE8000000UL)  d_ptr[1] = 0xE8000000UL;

                            /* point to start of requested channel for each chunk of traces */
                            ul_ptr = (unsigned long *)( (char *)v_ptr +
                                                         ( (i * disk_chunk_bytes) +
                                                           trace_offset ) );

                            /* add remaining samps into stacking trace */
                            for (samp=2; samp<num_samps; samp++)
                                d_ptr[samp] += (double)ul_ptr[samp] - 0x80000000UL;
                        }

                        /* set 2st sample of stacking trace */
                        if (d_ptr[1] != 0xE8000000UL)  d_ptr[1] = 0xF1000000UL;

                        /* calculate average */
                        for (samp=2; samp<num_samps; samp++)
                        {    d_ptr[samp] /= stack;
                            d_ptr[samp] += 0x80000000L;
                            d_ptr[samp]  = _LIMIT(0.0,d_ptr[samp],0xFFFFFFFFL);
                        }

                        /* point to start of trace to save */
                        ul_ptr = (unsigned long *)((char *)v_ptr + trace_offset);

						/* copy stacked trace into v_ptr */
                        for (samp=0; samp<num_samps; samp++)
                            ul_ptr[samp] = d_ptr[samp];

                        break;
                    }
                }   /* end of switch (rh_dtype) block */

                fwrite((void *)(((char *)v_ptr) + trace_offset),(long)(disk_trace_bytes),1,outfile);
                if (ferror(infile) || ferror(outfile)) break; /* out of for loop */
            }   /* end of for (trace=start; ...) block */
            if (ferror(infile) || ferror(outfile))
            {   printf("GPR_STAK - ERROR: ferror reported at trace %d\n",trace);
        ...
        {   char hd_infilename[MAX_PATHLEN],hd_outfilename[MAX_PATHLEN];
            ...
            int  i,itemp,print_it,dot,icount,samp;
            ...
            block_size = trace_size * stack;
            v_ptr = (void *)malloc(block_size);
            if (v_ptr == NULL)
            {   printf("GPR_STAK - ERROR: Unable to allocate storage for DT1 data block.\n");
                exit(4);
            }
            ...
            step_size  = HdInfo.step_size * stack;
            num_traces = 2 + ((end - 1 - start)/stack);
            traces_per_sec = 0.0;
            meters_per_mark = 0.0;
            num_gain_pts = 0;
            gain_pts = NULL;
            ...
            /* Condense file.  File pointer must be at start of first trace
               to get.  Read in desired trace plus stacked ones. */
            dot = (end-start)/stack/10;
            if (dot < 1) dot = 1;
            icount = 0;
            trace = start;
            ...
            {   if (!(icount % dot)) printf(".");
                ...
                /* add traces together */
                for (i=0; i<stack; i++)
                {   /* point to data area of each trace to stack */
                    s_ptr = (short *)( (char *)v_ptr +( (i * trace_size) +
                                                         hdr_block_bytes ) );

                    /* sum each sample into stacking trace */
                    for (samp=0; samp<num_samps; samp++)
                        d_ptr[samp] += s_ptr[samp];
                }
				/* calculate average */
                for (samp=0; samp<num_samps; samp++)
                {    d_ptr[samp] /= stack;
                    d_ptr[samp]  = _LIMIT(-32768.0,d_ptr[samp],32767.0);
                }

                /* point to data area of 1st trace */
                s_ptr = (short *)( (char *)v_ptr + hdr_block_bytes );

                /* copy stacked trace into v_ptr */
                for (samp=0; samp<num_samps; samp++)
                    s_ptr[samp] = d_ptr[samp];
        ...
        {   char *proc_hist,str[80];
            int  proc_hist_size = 160;
            int  i,itemp,dot,icount,samp;
            int  num_samps,data_block_bytes,hdr_block_bytes,num_gain_pts;
            short *s_ptr;
            long  *l_ptr;
            void  *v_ptr;          /* pointer to buffer for chunk of trace */
            float *f_ptr;
            double *d_ptr,l_limit,u_limit;
            double start_pos,final_pos,step_size,*gain_pts,traces_per_sec,meters_per_mark;
            int swap_bytes;
            long num_traces,offset,trace;
            size_t block_size,trace_size;
            ...
            {   case 1:  case 2:  case 4:
                ...
                case 3:
                    ...
                    break;
                default:
                    printf("GPR_STAK - ERROR: Unvalid SEG-Y sample format code.\n");
                    exit(4);
            }
            ...
            v_ptr = (void *)malloc(block_size);
            if (v_ptr == NULL)
            {   printf("GPR_STAK - ERROR: Unable to allocate storage for SEG-Y data block.\n");
                exit(4);
            }
            ...
            step_size  = GprInfo.position_step_size * stack;
            num_traces = 2 + ((end - 1 - start)/stack);
            ...
            {   if (!(icount % dot)) printf(".");
                ...
                {   switch (ReelHdr.format)
                    {   case 1:
                            /* point to data area of each trace to stack */
                            f_ptr = (float *)( (char *)v_ptr +( (i * trace_size) +
                                                         hdr_block_bytes ) );
                            /* sum each sample into stacking trace */
                            for (samp=0; samp<num_samps; samp++)
                                d_ptr[samp] += f_ptr[samp];
                            break;
                        case 2:  case 4:
                            /* point to data area of each trace to stack */
                            l_ptr = (long *)( (char *)v_ptr +( (i * trace_size) +
                                                         hdr_block_bytes ) );
                            /* sum each sample into stacking trace */
                            for (samp=0; samp<num_samps; samp++)
                                d_ptr[samp] += l_ptr[samp];
                            break;
                        case 3:
                            /* point to data area of each trace to stack */
                            s_ptr = (short *)( (char *)v_ptr +( (i * trace_size) +
                ...
                for (samp=0; samp<num_samps; samp++)
				{    d_ptr[samp] /= stack;
                    d_ptr[samp]  = _LIMIT(l_limit,d_ptr[samp],u_limit);
                }
                ...
                {   case 1:
                        /* point to data area of 1st trace */
                        f_ptr = (float *)( (char *)v_ptr + hdr_block_bytes );
                        for (samp=0; samp<num_samps; samp++)
                            f_ptr[samp] = d_ptr[samp];
                        break;
                    case 2:  case 4:
                        /* point to data area of 1st trace */
                        l_ptr = (long *)( (char *)v_ptr + hdr_block_bytes );
                        for (samp=0; samp<num_samps; samp++)
                            l_ptr[samp] = d_ptr[samp];
                        break;
                    case 3:
                        /* point to data area of 1st trace */
                        s_ptr = (short *)( (char *)v_ptr + hdr_block_bytes );
		...
		default:
            puts("\nGPR_STAK - ERROR: GPR file storage format is not recognized.");
