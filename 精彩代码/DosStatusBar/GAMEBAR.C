...
void men(windows_ptr win)
{
	...
	if(win->status==MIN)
	{
		...
		Object_Init(now,BUTTON,WINDOWS_X+2,WINDOWS_Y-BUTTON_Y*2,BUTTON_X-1,BUTTON_Y-1,"game",0,24,0,"",1,bt1,NOACTIVE);
		now->father=win;
		Behind_Object(now);
		Draw_Windows(now);
		last->next=now;
...
void Build_Object()
{
	...
	Object_Init(now,BAR,WINDOWS_X,WINDOWS_Y,WINDOWS_X_MOVE,WINDOWS_Y_MOVE,"This is a game desktop",0,24,0,"",1,win,ACTIVE);
	Behind_Object(now);
	win_now=now;
	Draw_Windows(now);
	head=now;
	now=(struct windows *)malloc(sizeof(struct windows));
	Object_Init(now,BUTTON,WINDOWS_X+2,WINDOWS_Y+2,BUTTON_X-1,BUTTON_Y-1,"Star",0,24,MIN,"",0,men,ACTIVE);//start is 5 word which is too long
	Draw_Windows(now);
	pre->next=now;
	pre=now;
	now=(struct windows *)malloc(sizeof(struct windows));
